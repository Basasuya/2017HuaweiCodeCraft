...
void Transform(int x) {
	...
	for(int i = head[x]; ~i; i = edge[i].next) {
		...
		double t2;
		if(LevelOfNodes[to] == 0) t2 = 0.1;
		else if(LevelOfNodes[to] == 1) t2 = 0.1;
		else if(LevelOfNodes[to] == 2) t2 = 0.1;
		else t2 = 0;
...
void GetNext() {
	...
	for(int i = 1; i <= netNum; ++i) {
		if(cdn[i]) {
			double t2 = 2*edge[netSt[i]].cap - edge[netSt[i]].flow;
			randch[++cnt].first = t2;
			randch[cnt].second = i;
			all += t2;
		}
	...
	t1 = randch[chpos[0]].second;

	if(ratio > 0.33 && netNum > 800) {
		randnum = (rand() / (RAND_MAX + 1.0));
		chpos[1] = cnt;
		for(int i = 1; i <= cnt; ++i) {
			if(prefix[i] > randnum && i != chpos[0]) {
				chpos[1] = i; break;
			}
		}
		t2 = randch[chpos[1]].second;
	}

	if(ratio > 0.66 && netNum > 800) {
		randnum = (rand() / (RAND_MAX + 1.0));
		chpos[2] = cnt;
		for(int i = 1; i <= cnt; ++i) {
			if(prefix[i] > randnum && i != chpos[0] && i != chpos[1]) {
				chpos[2] = i; break;
			}
		}
		t3 = randch[chpos[2]].second;
	}

	if(choiceoflife < ratio ) {
		IsDeleteNode = true;
		cdn[t1] = 0;
		if(chpos[1] != chpos[0] && ratio > 0.33 && netNum > 800)
			cdn[t2] = 0;
		if(chpos[2] != chpos[0] && chpos[2] != chpos[1] && ratio > 0.66 && netNum > 800)
			cdn[t3] = 0;
	}else {
		IsDeleteNode = false;
		Transform(t1);
		/*
		if(chpos[1] != chpos[0] && ratio > 0.5 && netNum > 800)
			Transform(t2);
		if(chpos[2] != chpos[0] && chpos[2] != chpos[1] && ratio > 0.8 && netNum > 800)
			Transform(t3);
			*/
	}
	MCMF(s, t);
