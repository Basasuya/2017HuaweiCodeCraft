...
void Choose() {
  ...
  for(int i = 1; i <= tot; ++i) {
    ...
    if(gene[chonum].flag == 0 && randnum < 0.5) {
      continue; --i;
    }
    for(int j = 1; j <= netNum; ++j) chosen[i][j] = gene[chonum].cdn[j];
  }
}
void Across() {
...
void deploy_server(char * topo[MAX_EDGE_NUM], int line_num,char * filename)
{
	...
	stTime = clock();
	Init();
	char TransInteger[10]; int cnt = 0;
	int cnroute = 0, cnuse = 0;
		...
		if(netNum == -1) {
			for(int j = 0; topo[i][j]; ++j) {
				if(topo[i][j] >= '0' && topo[i][j] <= '9') {
					TransInteger[cnt++] = topo[i][j];
				}else if(cnt) {
					TransInteger[cnt] = 0; int tt = atoi(TransInteger); cnt = 0;

					if(netNum == -1) netNum = tt;
					else if(routeNum == -1) routeNum = tt;
					else useNum = tt;
			...
			for(int j = 0; topo[i][j]; ++j) {
				if(topo[i][j] >= '0' && topo[i][j] <= '9') {
					...
					else if(cap == -1) cap = tt;
					else cost = tt;
				}
			}
			if(fr != -1) {
				fr ++; to ++;
				Addedge(fr, to, cap, cost), Addedge(to, fr, cap, cost);
				cnroute ++;
			}
		}else if(cnuse < useNum) {
			int fr=-1,to=-1,cap=-1;
			for(int j = 0; topo[i][j]; ++j) {
				if(topo[i][j] >= '0' && topo[i][j] <= '9') {
					TransInteger[cnt++] = topo[i][j];
			...
			if(fr != -1) {
				fr ++; to ++;
				Son[to] = fr; Need[fr] = cap; Dad[fr] = to;
				cnuse ++;
			}
		}else break;
	}
	...
	solve();
	printf("%d\n", Ans);
	Print(PathAns.size()); Print(-1);
	for(int i = 0; i < (int)PathAns.size(); ++i) {
		Print(-1);
		int last;
		for(int j = 0; j < (int)PathAns[i].size(); ++j) {
			if(j) Print(-2);
			Print(PathAns[i][j]);
		}
	}
