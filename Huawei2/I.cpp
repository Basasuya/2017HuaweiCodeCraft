...
void solve() {
  ...
  for(int i = 1; i <= netNum; ++i) {
	...
	printf("%d\n", netCost);
	double t1 = Maxflow*1.0 / useNum; double t2 = netCost*1.0;
	printf("usenum:%d flow:%.2f netcost:%.2f server:%d\n", useNum,t1,t2,tt);
}
...
int main(){
	...
	stTime = clock();
	Init();
	freopen("/home/basasuya/2/case7.txt","r",stdin);
