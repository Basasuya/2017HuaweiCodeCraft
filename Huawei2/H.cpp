...
void MCMF(int s,int t){
	...
	int po = Hash();
	if(po != -1) {
		for(int i = 1; i <= netNum; ++i) edge[netSt[i]].flow = hashmp[po].flow[i];
		tmpAns = hashans[po];
  ...
  while(1) {
	...
	hashans.push_back(tmpAns);
	Fode tt;
	for(int i = 1; i <= netNum; ++i) tt.flow[i] = edge[netSt[i]].flow;
	hashmp.push_back(tt);
	mp[nowHash] = hashans.size()-1;
...
int main(){
	...
	stTime = clock();
	Init();
	freopen("/home/basasuya/case_example/2/case1.txt","r",stdin);
	while(~scanf("%d %d %d",&netNum, &routeNum, &useNum)) {
		...
		if(netNum < 300) netCost = 600;
		else if(netNum < 800) netCost = 800;
		else netCost = 1000;

		solve();
		printf("MACMF for %d times\n", ccc);
		memset(cdn, 0, sizeof(cdn));
