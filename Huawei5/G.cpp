...
void Transform(int x) {
 ...
 int chpos = rand() % cnt + 1;
	int pos = randch[chpos];
	cdn[x] = 0; cdn[pos] = 1;
}
  ...
  double randnum;
	...
	for(int j = 0 ; j < NUM ; j ++) {
		if(ratio > j * _NUM) {
			...
			{
				flag ++;
				cdn[randch[chpos[j]]] = 0;
			}
		}
	}

  if(!flag) {
		Transform(randch[chpos[0]]);
	}
}
...
int chooseLevel(int level, int tt) {
    LEVEL = level;
    tem = T;
		memset(cdn,0,sizeof(cdn));
...
void solve() {
  ...
  int LEVEL1, LEVEL2;
	...
	if(netNum < 800){
		timeofstep0 = 6; LEVEL1 = 5; LEVEL2 = 5; numofstep0 = 2;
	}else {
		timeofstep0 = 7; LEVEL1 = 5; LEVEL2 = 6; numofstep0 = 1;
	}
  ...
  for(int i = 1; i <= netNum; ++i) {
	...
	double tmpT = 0;
  int times = 0;
	int t1 = chooseLevel(LEVEL1, timeofstep0);
  int t2 = chooseLevel(LEVEL2, timeofstep0);

  LEVEL = t1 < t2? LEVEL1 : LEVEL2;
  printf("%d\n", LEVEL);

	for(int i = 1; i <= numofstep0; ++i){
    tem = T;
		memset(cdn,0,sizeof(cdn));
		for(int j = 1; j <= useNum; ++j) {
			cdn[Dad[j]] = 1;
